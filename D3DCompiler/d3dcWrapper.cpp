...
void InitializeDLL()
{
	if (!gInitialized)
	{
		...
		if (GetPrivateProfileInt(L"Logging", L"unbuffered", 0, dir))
		{
			...
			LogInfo("[INF]:  unbuffered=1  return: %d\n", unbuffered);
		}
		...
		if (GetPrivateProfileInt(L"Logging", L"force_cpu_affinity", 0, dir))
		{
			...
			LogInfo("[INF]: CPU Affinity forced to 1- no multithreading: %s\n", result ? "true" : "false");
		}
...
static void InitC46()
{
    ...
    if (!hC46)
    {
        LogInfo("[INF]: LoadLibrary on D3DCompiler_" COMPILER_DLL_VERSION "_org.dll failed\n");
    ...
    if (!hD3D11)
    {
        LogInfo("[INF]: LoadLibrary on d3d11.dll wrapper failed\n");
    }
...
{
	InitC46();
	LogInfo("[INF]: D3DAssemble called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DDisassemble11Trace called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DReadFileToBlob called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DWriteBlobToFile called\n"); 
           ...
           _Out_opt_ ID3DBlob** ppErrorMsgs)
{
	...
	{
		LogInfo("[INF]: D3DCompile called with\n"); 
		LogInfo("[INF]:  SourceName = %s\n", pSourceName);
		LogInfo("[INF]:  Entrypoint = %s\n", pEntrypoint);
		LogInfo("[INF]:  Target = %s\n", pTarget);
		LogInfo("[INF]:  ppErrorMsgs = %p\n", ppErrorMsgs);
	...
	if (ret == S_OK && ppCode && *ppCode && !wrapperCall)
	{
		...
		LogInfo("[INF]:    Filename = %s\n", shaderName);
		LogInfo("[INF]:    Compiled bytecode size = %d, bytecode handle = %p\n", (*ppCode)->GetBufferSize(), (*ppCode)->GetBufferPointer());
			...
			else
			{
				...
				if (hFind != INVALID_HANDLE_VALUE)
				{
					...
					if (LogFile)
					{
						...
						LogInfo("[INF]:    Replacement shader found. Loading replacement HLSL code from file \"%s\".\n", path);
	...
	//	{
	//		LogInfo("[INF]:    sending code hash to d3d11.dll wrapper failed\n");
	//	}
	//}
	return ret;
}
...
{
	InitC46();
	LogInfo("[INF]: D3DCompile2 called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DCompileFromFile called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DPreprocess called\n"); 
...
{
	InitC46();
	//LogInfo("[INF]: D3DDisassemble called\n"); 
	//
...
{
	InitC46();
	LogInfo("[INF]: D3DDisassembleRegion called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DDisassemble10Effect called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DGetTraceInstructionOffsets called\n"); 
...
{
	InitC46();
	//LogInfo("[INF]: D3DGetInputSignatureBlob called\n"); 
	//
...
{
	InitC46();
	LogInfo("[INF]: D3DGetOutputSignatureBlob called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DGetInputAndOutputSignatureBlob called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DStripShader called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DGetBlobPart called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DSetBlobPart called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DCompressShaders called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DDecompressShaders called\n"); 
...
{
	InitC46();
	LogDebug("[DBG]: D3DCreateBlob called\n");	// Too chatty for LogInfo
...
{
	InitC46();
	LogInfo("[INF]: D3DCompileFromMemory called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DDisassembleCode called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DDisassembleEffect called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DGetCodeDebugInfo called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DPreprocessFromMemory called\n"); 
...
{
	InitC46();
	LogInfo("[INF]: D3DReflectCode called\n"); 
