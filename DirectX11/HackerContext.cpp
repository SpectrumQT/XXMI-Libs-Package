...
HackerContext* HackerContextFactory(ID3D11Device1 *pDevice1, ID3D11DeviceContext1 *pContext1)
{
	...
	if (G->hunting || gLogDebug) {
		LogInfo("[INF]:  Creating FrameAnalysisContext\n");
		return new FrameAnalysisContext(pDevice1, pContext1);
	}

	LogInfo("[INF]:  Creating HackerContext - frame analysis log will not be available\n");
...
void HackerContext::RecordRenderTargetInfo(ID3D11RenderTargetView *target, UINT view_num)
{
	...
	LogDebug("[DBG]:  View #%d, Format = %d, Is2D = %d\n",
...
void HackerContext::ProcessShaderOverride(ShaderOverride *shaderOverride, bool isPixelShader, DrawContext *data)
{
	...
	LogDebug("[DBG]:  override found for shader\n");
...
void HackerContext::DeferredShaderReplacement(ID3D11DeviceChild *shader, UINT64 hash, wchar_t *shader_type)
{
	...
	case ShaderRegexCache::NO_MATCH:
		LogInfo("[INF]: %S %016I64x has cached ShaderRegex miss\n", shader_type, hash);
		goto out_drop;
	case ShaderRegexCache::MATCH:
		LogInfo("[INF]: Loaded %S %016I64x command list from ShaderRegex cache\n", shader_type, hash);
		goto out_drop;
	case ShaderRegexCache::PATCH:
		LogInfo("[INF]: Loaded %S %016I64x bytecode from ShaderRegex cache\n", shader_type, hash);
		break;
	case ShaderRegexCache::NO_CACHE:
		LogInfo("[INF]: Performing deferred shader analysis on %S %016I64x...\n", shader_type, hash);
		...
		try {
			...
			LogInfo("[INF]:    *** Exception while patching shader\n");
			goto out_drop;
		}

		if (!patch_regex) {
			LogInfo("[INF]: Patch did not apply\n");
			goto out_drop;
		}
		...
		try {
			...
			if (FAILED(hr)) {
				LogInfo("[INF]:    *** Assembling patched shader failed\n");
				goto out_drop;
			}
	...
	if (FAILED(hr)) {
		LogInfo("[INF]:    *** Creating replacement shader failed\n");
		goto out_drop;
	}
...
void HackerContext::BeforeDraw(DrawContext &data)
{
	...
	if (G->hunting == HUNTING_MODE_ENABLED)
	{
		...
		EnterCriticalSectionPretty(&G->mCriticalSection);
		{
			...
			{
				LogDebug("[DBG]:  Skipping selected operation. CurrentIndexBuffer = %08lx, CurrentVertexShader = %016I64x, CurrentPixelShader = %016I64x\n",
...
STDMETHODIMP_(ULONG) HackerContext::Release(THIS)
{
	...
	LogDebug("[DBG]: HackerContext::Release counter=%d, this=%p\n", ulRef, this);
	...
	if (ulRef <= 0)
	{
		...
		if (mHackerDevice != nullptr) {
			if (mHackerDevice->GetHackerContext() == this) {
				LogInfo("[INF]:  clearing mHackerDevice->mHackerContext\n");
				mHackerDevice->SetHackerContext(nullptr);
			}
		} else
			LogInfo("[INF]: HackerContext::Release - mHackerDevice is NULL\n");
...
{
	LogDebug("[DBG]: HackerContext::QueryInterface(%s@%p) called with IID: %s\n", type_name(this), this, NameFromIID(riid).c_str());
	...
	if (FAILED(hr))
	{
		LogDebug("[DBG]:  failed result = %x for %p\n", hr, ppvObject);
		return hr;
	}
	...
	{
		*ppvObject = this;
		LogDebug("[DBG]:  return HackerContext(%s@%p) wrapper of %p\n", type_name(this), this, mOrigContext1);
	}
	else if (riid == __uuidof(ID3D11DeviceContext1))
	{
		if (!G->enable_platform_update) 
		{
			LogInfo("[INF]: ***  returns E_NOINTERFACE as error for ID3D11DeviceContext1 (try allow_platform_update=1 if the game refuses to run).\n");
			*ppvObject = NULL;
...
{
	LogDebug("[DBG]: HackerContext::GetDevice(%s@%p) returns %p\n", type_name(this), this, mHackerDevice);
...
{
	LogDebug("[DBG]: HackerContext::GetPrivateData(%s@%p) called with IID: %s\n", type_name(this), this, NameFromIID(guid).c_str());

	HRESULT hr = mOrigContext1->GetPrivateData(guid, pDataSize, pData);
	LogDebug("[DBG]:  returns result = %x, DataSize = %d\n", hr, *pDataSize);
...
{
	LogInfo("[INF]: HackerContext::SetPrivateData(%s@%p) called with IID: %s\n", type_name(this), this, NameFromIID(guid).c_str());
...
{
	LogInfo("[INF]: HackerContext::SetPrivateDataInterface(%s@%p) called with IID: %s\n", type_name(this), this, NameFromIID(guid).c_str());
		...
		D3D11_MAPPED_SUBRESOURCE *pMappedResource)
{
	...
	if (!replace) {
		LogInfo("[INF]: TrackAndDivertMap out of memory\n");
		goto out_profile;
	}
		...
		UINT *replaceDstX, D3D11_BOX *replaceBox)
{
	...
	LogDebug("[DBG]: CopySubresourceRegion %08lx (%u:%u x %u:%u / %u x %u) -> %08lx (%u x %u / %u x %u)\n",
	...
	*currentShaderHandle = pShader;
	...
	if (pShader) {
		...
		if (!G->mShaderOverrideMap.empty() || !shader_regex_groups.empty() || (G->hunting == HUNTING_MODE_ENABLED)) {
			...
			if (i != G->mShaders.end()) {
				*currentShaderHash = i->second;
				LogDebug("[DBG]:  shader found: handle = %p, hash = %016I64x\n", *currentShaderHandle, *currentShaderHash);
				...
				LogDebug("[DBG]:  shader %p not found\n", pShader);
		} else {
		...
		if (it != G->mReloadedShaders.end() && it->second.replacement != NULL) {
			LogDebug("[DBG]:  shader replaced by: %p\n", it->second.replacement);
...
{
	if (!mHackerDevice) {
		LogInfo("[INF]:  error querying device. Can't set INI parameters texture.\n");
		return;
	}

	// Set constants from ini file if they exist
	if (mHackerDevice->mIniResourceView && G->IniParamsReg >= 0) {
		LogDebug("[DBG]:  adding ini constants as texture to shader resources in slot %i.\n", G->IniParamsReg);
...
void HackerContext::InitIniParams()
{
	...
	if (mOrigContext1->GetType() != D3D11_DEVICE_CONTEXT_IMMEDIATE) {
		LogInfo("[INF]: BUG: InitIniParams called on a deferred context\n");
		DoubleBeepExit();
	}
	...
	if (mHackerDevice->mIniTexture) {
		...
		if (SUCCEEDED(hr)) {
			...
			LogInfo("[INF]: InitIniParams: Map failed\n");
		}
	}
		...
		ID3D11ShaderResourceView *const *ppShaderResourceViews)
{
	...
	if (mHackerDevice->mIniResourceView && G->IniParamsReg >= 0) {
		if (NumViews > G->IniParamsReg - StartSlot) {
			LogDebug("[DBG]:  Game attempted to unbind IniParams, pinning in slot %i\n", G->IniParamsReg);
			if (!override_srvs) {
	...
	UINT NumClassInstances)
{
	...
	if (pPixelShader) {
		...
		{
			LogDebug("[DBG]:  adding Z buffer to shader resources in slot 126.\n");
