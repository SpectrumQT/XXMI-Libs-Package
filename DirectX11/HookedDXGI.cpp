...
static HackerDevice* prepare_devices_for_dx12_warning(IUnknown *unknown_device)
{
	...
	LogInfo("[INF]: Preparing to enable D3D11On12 compatibility mode for overlay...\n");

	if (FAILED(d3d12_queue->GetDevice(IID_ID3D12Device, (void**)&d3d12_device)))
		goto out;

	LogInfo("[INF]: ID3D12Device: %p\n", d3d12_device);
	...
	if (FAILED(hr)) {
		LogInfo("[INF]: D3D11On12CreateDevice failed: 0x%x\n", hr);
		goto out;
	}

	LogInfo("[INF]: ID3D11Device: %p\n", d3d11_device);
	LogInfo("[INF]: ID3D11DeviceContext: %p\n", d3d11_context);
	...
	LogInfo("[INF]: HackerDevice: %p\n", dev_wrap);
	LogInfo("[INF]: HackerContext: %p\n", context_wrap);
...
static HackerDevice* sort_out_swap_chain_device_mess(IUnknown **device)
{
	...
	if (hackerDevice) {
		...
		*device = (hackerDevice)->GetPossiblyHookedOrigDevice1();
		...
		LogInfo("[INF]: WARNING: Could not locate HackerDevice for %p\n", *device);
		...
		if (check_interface_supported(*device, IID_ID3D11Device)) {
			...
			LogInfo("[INF]: BUG: Unwrapped ID3D11Device!\n");
			DoubleBeepExit();
		}

		LogInfo("[INF]: FATAL: Unsupported DirectX Version!\n");
		...
		DXGI_SWAP_CHAIN_DESC *origSwapChainDesc)
{
	...
	if (G->SCREEN_UPSCALING == 0)		// Normal case
	{
		...
		LogInfo("[INF]:  HackerSwapChain %p created to wrap %p\n", swapchainWrap, origSwapChain);
	}
	else								// Upscaling case
	{
		...
		LogInfo("[INF]:  HackerUpscalingSwapChain %p created to wrap %p.\n", swapchainWrap, origSwapChain);
	...
	*ppSwapChain = swapchainWrap;
	...
	LogInfo("[INF]: -> HackerSwapChain = %p wrapper of ppSwapChain = %p\n", swapchainWrap, origSwapChain);
}
	...
	_Out_  IDXGISwapChain1 **ppSwapChain)
{
	if (get_tls()->hooking_quirk_protection) {
		LogInfo("[INF]: Hooking Quirk: Unexpected call back into IDXGIFactory2::CreateSwapChainForHwnd, passing through\n");
		// No known cases
	...
	LogInfo("[INF]: *** Hooked IDXGIFactory2::CreateSwapChainForHwnd(%p) called\n", This);
	LogInfo("[INF]:  Device = %p\n", pDevice);
	LogInfo("[INF]:  SwapChain = %p\n", ppSwapChain);
	LogInfo("[INF]:  Description1 = %p\n", pDesc);
	LogInfo("[INF]:  FullScreenDescription = %p\n", pFullscreenDesc);
	...
	if (FAILED(hr))
	{
		LogInfo("[INF]: ->Failed result %#x\n\n", hr);
		goto out_release;
	}

	wrap_factory2_swap_chain(hackerDevice, ppSwapChain);

	LogInfo("[INF]: ->return result %#x\n", hr);
out_release:
	if (hackerDevice)
...
{
	if (get_tls()->hooking_quirk_protection) {
		LogInfo("[INF]: Hooking Quirk: Unexpected call back into IDXGIFactory2::CreateSwapChainForCoreWindow, passing through\n");
		// No known cases
	...
	LogInfo("[INF]: *** Hooked IDXGIFactory2::CreateSwapChainForCoreWindow(%p) called\n", This);
	LogInfo("[INF]:  Device = %p\n", pDevice);
	LogInfo("[INF]:  SwapChain = %p\n", ppSwapChain);
	LogInfo("[INF]:  Description1 = %p\n", pDesc);
...
{
	if (get_tls()->hooking_quirk_protection) {
		LogInfo("[INF]: Hooking Quirk: Unexpected call back into IDXGIFactory2::CreateSwapChainForComposition, passing through\n");
		// No known cases
	...
	LogInfo("[INF]: *** Hooked IDXGIFactory2::CreateSwapChainForComposition(%p) called\n", This);
	LogInfo("[INF]:  Device = %p\n", pDevice);
	LogInfo("[INF]:  SwapChain = %p\n", ppSwapChain);
	LogInfo("[INF]:  Description1 = %p\n", pDesc);
...
static void HookFactory2CreateSwapChainMethods(IDXGIFactory2* dxgiFactory)
{
	...
	LogInfo("[INF]: *** IDXGIFactory2 creating hooks for CreateSwapChain variants. \n");
	...
	if (dwOsErr == ERROR_SUCCESS)
		LogInfo("[INF]:  Successfully installed IDXGIFactory2->CreateSwapChainForHwnd hook.\n");
	else
		LogInfo("[INF]:  *** Failed install IDXGIFactory2->CreateSwapChainForHwnd hook.\n");
	...
	if (dwOsErr == ERROR_SUCCESS)
		LogInfo("[INF]:  Successfully installed IDXGIFactory2->CreateSwapChainForCoreWindow hook.\n");
	else
		LogInfo("[INF]:  *** Failed install IDXGIFactory2->CreateSwapChainForCoreWindow hook.\n");
	...
	if (dwOsErr == ERROR_SUCCESS)
		LogInfo("[INF]:  Successfully installed IDXGIFactory2->CreateSwapChainForComposition hook.\n");
	else
		LogInfo("[INF]:  *** Failed install IDXGIFactory2->CreateSwapChainForComposition hook.\n");
}
...
{
	if (get_tls()->hooking_quirk_protection) {
		LogInfo("[INF]: Hooking Quirk: Unexpected call back into IDXGIFactory::CreateSwapChain, passing through\n");
	...
	LogInfo("[INF]:  CreateSwapChain returned handle = %p\n", retChain);
	analyse_iunknown(retChain);

	wrap_swap_chain(hackerDevice, ppSwapChain, pDesc, &origSwapChainDesc);

	LogInfo("[INF]: ->IDXGIFactory::CreateSwapChain return result %#x\n\n", hr);
out_release:
	if (hackerDevice)
		hackerDevice->Release();
	return hr;
...
{
	LogInfo("[INF]: *** IDXGIFactory creating hook for CreateSwapChain. \n");
	...
	if (dwOsErr == ERROR_SUCCESS)
		LogInfo("[INF]:  Successfully installed IDXGIFactory->CreateSwapChain hook.\n");
	else
		LogInfo("[INF]:  *** Failed install IDXGIFactory->CreateSwapChain hook.\n");
}
...
{
	LogInfo("[INF]: *** Hooked_CreateDXGIFactory called with riid: %s\n", NameFromIID(riid).c_str());
	...
	{
		LogInfo("[INF]:  returns E_NOINTERFACE as error for IDXGIFactory2.\n");
		*ppFactory = NULL;
	...
	if (SUCCEEDED(res))
	{
		...
		*ppFactory = (void*)dxgiFactory;
		LogInfo("[INF]:  Upcast QueryInterface(IDXGIFactory2) returned result = %x, factory = %p\n", res, dxgiFactory);

		if (!fnOrigCreateSwapChainForHwnd)
			HookFactory2CreateSwapChainMethods(dxgiFactory);
	}

	LogInfo("[INF]:  CreateDXGIFactory returned factory = %p, result = %x\n", *ppFactory, hr);
	return hr;
}
...
{
	LogInfo("[INF]: *** Hooked_CreateDXGIFactory1 called with riid: %s\n", NameFromIID(riid).c_str());
	...
	LogInfo("[INF]:  CreateDXGIFactory1 returned factory = %p, result = %x\n", *ppFactory1, hr);
	return hr;
}
...
{
	LogInfo("[INF]: *** Hooked_CreateDXGIFactory2 called with riid: %s\n", NameFromIID(riid).c_str());
	...
	LogInfo("[INF]:  CreateDXGIFactory2 returned factory = %p, result = %x\n", *ppFactory2, hr);
	return hr;
}
...
//	if (LogFile)
//	{
//		LogInfo("[INF]: HackerDXGIFactory::MakeWindowAssociation(%s@%p) called with WindowHandle = %p, Flags = %x\n", type_name(this), this, WindowHandle, Flags);
...
//	{
//		LogInfo("[INF]:  overriding Flags to allow all window commands\n");
//
//		Flags = 0;
//	}
...
//{
//	LogInfo("[INF]: HackerDXGIOutput::GetDisplayModeList(%s@%p) called\n", type_name(this), this);
//
...
//			if (FilterRate(rate))
//			{
//				LogInfo("[INF]:  Skipping mode: width=%d, height=%d, refresh rate=%f\n", pDesc[j].Width, pDesc[j].Height,
...
//			else
//			{
//				LogInfo("[INF]:  Mode detected: width=%d, height=%d, refresh rate=%f\n", pDesc[j].Width, pDesc[j].Height,
//					(float)pDesc[j].RefreshRate.Numerator / (float)pDesc[j].RefreshRate.Denominator);
//			}
//		}
//	}
...
//{
//	if (pModeToMatch) LogInfo("[INF]: HackerDXGIOutput::FindClosestMatchingMode(%s@%p) called: width=%d, height=%d, refresh rate=%f\n", type_name(this), this,
...
//	if (pClosestMatch) LogInfo("[INF]:  returning width=%d, height=%d, refresh rate=%f\n",
