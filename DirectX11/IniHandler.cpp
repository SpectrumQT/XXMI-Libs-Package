...
bool check_include_condition(wstring *val, const wstring *ini_namespace)
{
	...
	if (!ret)
		LogInfo("[INF]:        condition = false, skipping \"%S\"\n", ini_namespace->c_str());
...
static bool ParseIniPreamble(wstring *wline, wstring *ini_namespace)
{
	...
	if (delim != wline->npos) {
		...
		if (!_wcsicmp(key.c_str(), L"namespace")) {
			LogInfo("[INF]:        Renaming namespace \"%S\" -> \"%S\"\n", ini_namespace->c_str(), val.c_str());
			*ini_namespace = val;
			return true;
		}
	}
...
static pcre2_code* glob_to_regex(wstring &pattern)
{
	...
	if (pcre2_pattern_convert((PCRE2_SPTR)apattern.c_str(),
		...
		LogInfo("[INF]: Bad pattern: exclude_recursive=%S\n", pattern.c_str());
		return NULL;
	}

	regex = pcre2_compile(converted, blength, PCRE2_CASELESS, &err, &err_off, NULL);
	if (!regex)
		LogInfo("[INF]: WARNING: exclude_recursive PCRE2 regex compilation failed");
...
static void ParseIniFilesRecursive(wchar_t *migoto_path, const wstring &rel_path, vector<pcre2_code*> &exclude)
{
	...
	if (hFind == INVALID_HANDLE_VALUE) {
		LogInfo("[INF]:    Recursive include path \"%S\" not found\n", search_path.c_str());
		return;
	}

	do {
		if (matches_globbing_vector(find_data.cFileName, exclude)) {
			LogInfo("[INF]:    Excluding \"%S\"\n", find_data.cFileName);
			continue;
		}
		...
		if (find_data.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
			...
			LogDebug("[DBG]:    Not a directory or ini file: \"%S\"\n", find_data.cFileName);
		}
...
static void _GetIniSection(IniSections *custom_ini_sections, IniSectionVector **key_vals, const wchar_t *section)
{
	...
	try {
		*key_vals = &custom_ini_sections->at(section).kv_vec;
		...
		LogDebug("[DBG]: WARNING: GetIniSection() called on a section not in the ini_sections map: %S\n", section);
...
bool GetIniString(const wchar_t *section, const wchar_t *key, const wchar_t *def, std::string *ret)
{
	...
	if (!ret) {
		LogInfo("[INF]: BUG: Misuse of GetIniString()\n");
		DoubleBeepExit();
	}
		...
		wchar_t *whitelist[], bool register_command_lists=true)
{
	...
	if (!IsCommandListSection(id)) {
		LogInfoW(L"[INF]: BUG: ParseCommandList() called on a section not in the CommandListSections list: %s\n", id);
		DoubleBeepExit();
	}

	scope.emplace_front();

	LogDebug("[DBG]: Registering command list: %S\n", id);
	...
	for (entry = section->begin(); entry < section->end(); entry++) {
		...
		if (entry->ini_namespace == G->user_config && !G->user_config.empty()) {
			...
			LogInfoW(L"[INF]: WARNING: Unrecognised entry in %ls: %ls\n", G->user_config.c_str(), raw_line->c_str());
			continue;
		}
...
static void ParseConstantsSection()
{
	...
	for (next = section->begin(), entry = next; entry < section->end(); entry = next) {
		...
		if (val->empty())
			LogInfo("[INF]:  global %S\n", name.c_str());
		else
			LogInfo("[INF]:  global %S=%f\n", name.c_str(), fval);
...
static bool parse_shader_regex_section_pattern(const std::wstring *section_id, const std::wstring *pattern_id, ShaderRegexGroup *regex_group)
{
	...
	// \n or \s+ easier to spot.
	LogInfo("[INF]: --------- final pcre2 regex pattern used after ini parsing ---------\n");
	LogInfo("[INF]: %s\n", pattern.c_str());
	LogInfo("[INF]: --------------------------------------------------------------------\n");
...
static bool parse_shader_regex_section_replace(const std::wstring *section_id, const std::wstring *pattern_id, ShaderRegexGroup *regex_group)
{
	...
	LogInfo("[INF]: --------- final pcre2 replace string used after ini parsing ---------\n");
...
static void ParseShaderRegexSections()
{
	...
	LogInfo("[INF]: ShaderRegex hash: %08x\n", shader_regex_hash);
		...
		struct EnumName_t<const wchar_t *, T> *enum_names)
{
	...
	if (!setting.size() || !setting.compare(L"0")) {
		*val = 0;
		*mask = 0xffffffff;
		LogInfo("[INF]:    Using: 0x%08x / 0x%08x\n", *val, *mask);
		return true;
	}
...
static void ToggleFullScreen(HackerDevice *device, void *private_data)
{
	...
	LogInfo("[INF]: > full screen forcing toggled to %d (will not take effect until next mode switch)\n", G->SCREEN_FULLSCREEN);
}
...
static void ForceFullScreen(HackerDevice *device, void *private_data)
{
	...
	LogInfo("[INF]: > Switching to exclusive full screen mode\n");
...
void LoadConfigFile()
{
	...
	if (GetPrivateProfileInt(L"Logging", L"calls", 1, iniFile))
	{
		...
		LogInfo("[INF]: Game path: %S\n"
			"3DMigoto path: %S\n\n",
			exe_path, our_path);

		LogInfoW(L"[INF]: ----------- " INI_FILENAME L" settings -----------\n");
	}
	LogInfo("[INF]: [Logging]\n");
	LogInfo("[INF]:  calls=1\n");
	...
	if (LogFile && GetIniBool(L"Logging", L"unbuffered", false, NULL))
	{
		...
		LogInfo("[INF]:    unbuffered return: %d\n", unbuffered);
	}
	...
	if (GetIniBool(L"Logging", L"force_cpu_affinity", false, NULL))
	{
		...
		LogInfo("[INF]:    force_cpu_affinity return: %s\n", affinity ? "true" : "false");
	}
	...
	LogInfo("[INF]: [ClearRenderTargetView]\n");
	...
	LogInfo("[INF]: [ClearDepthStencilView]\n");
	...
	LogInfo("[INF]: [ClearUnorderedAccessViewUint]\n");
	...
	LogInfo("[INF]: [ClearUnorderedAccessViewFloat]\n");
...
void SavePersistentSettings()
{
	...
	if (!f) {
		LogInfo("[INF]: Unable to save settings in %S\n", G->user_config.c_str());
		return;
	}

	LogInfo("[INF]: Saving user settings to %S\n", G->user_config.c_str());
...
void ReloadConfig(HackerDevice *device)
{
	...
	LogInfoW(L"[INF]: Reloading " INI_FILENAME L"...\n");
	...
	if (mHackerContext) {
		if (G->iniParams.size() != G->iniParamsReserved) {
			LogInfo("[INF]:  Resizing IniParams from %Ii to %d\n", G->iniParams.size(), G->iniParamsReserved);
