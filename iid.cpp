...
static void check_interface(IUnknown *unknown, REFIID riid, char *iid_name, IUnknown *canonical)
{
	...
	if (test) {
		...
		if (canonical_test == canonical)
			LogInfo("[INF]:  Supports %s: %p\n", iid_name, test);
		else
			LogInfo("[INF]:  Supports %s: %p (COM identity violation: %p)\n", iid_name, test, canonical_test);
	} else
		LogDebug("[DBG]:  %s not supported\n", iid_name);
}
...
void analyse_iunknown(IUnknown *unknown)
{
	...
	LogInfo("[INF]: Checking what interfaces %p supports...\n", unknown);
	...
	LogInfo("[INF]:  Win 8.1 interfaces not checked (3DMigoto built with old SDK)\n");
#endif
#ifndef NTDDI_WIN10
	LogInfo("[INF]:  Win 10 & DX12 interfaces not checked (3DMigoto built with old SDK)\n");
#endif
#ifndef NTDDI_WIN10_RS3
	LogInfo("[INF]:  Win 10 RS3 interfaces not checked (3DMigoto built with old SDK)\n");
#endif
}
